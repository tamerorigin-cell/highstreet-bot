    ...
    try:
        ...
        for row in ws.iter_rows(min_row=3, values_only=True):
            if row[0] and str(row[0]).upper().replace(' ','') == uid:
                return {'unit': row[0], 'type': str(row[1]).strip(), 'indoor': round(row[3],2),
                        'outdoor': round(row[6],2) if row[6] else 0, 'rate': round(row[9])}
...
body{{font-family:'Palatino Linotype',Palatino,Georgia,serif;font-size:11px;}}
.hdr{{background:#2B1810;padding:22px 36px 18px;}}
.hdr-top{{display:flex;justify-content:space-between;}}
.brand{{font-size:24px;font-weight:700;letter-spacing:8px;color:#C9A84C;}}
...
.meta-valid{{font-size:9px;color:#B8A88A;}}
.div{{width:100%;height:1px;background:#C9A84C44;margin:10px 0 8px;}}
.prep{{font-size:7px;letter-spacing:3px;color:#C9A84C;margin-bottom:3px;}}
...
<div class="hdr">
  <div class="hdr-top">
    ...
    <div class="meta"><div class="meta-lbl">LEASING OFFER</div><div class="meta-date">21 May 2026</div><div class="meta-valid">Valid until 28 May 2026</div></div>
  </div>
  <div class="div"></div>
...
<div class="body">
  <div class="sec"><div class="sec-lbl">Unit Details</div>
    <table class="utbl">
      ...
      <tbody><tr><td>{bld}</td><td>{flr}</td><td>{unt}</td><td>Core &amp; Shell</td><td>{unit['indoor']} sqm</td><td>{"—" if unit['outdoor']==0 else str(unit['outdoor'])+" sqm"}</td></tr></tbody>
    </table>
  </div>
  ...
  <div class="sec"><div class="sec-lbl">Payment Structure</div>
    <div class="pbox">
      <div class="pgrid">
...
<div class="p2">
  <div class="sec"><div class="sec-lbl">General Terms</div>
    <div class="gt">
      ...
      <p>3. This offer is valid until 28 May 2026 and may be withdrawn or amended thereafter.</p>
  ...
  <div class="sec"><div class="sec-lbl">Acceptance</div>
    <div class="agrid">
      <div><div class="slbl">Tenant Acceptance</div>
      ...
      <div><div class="slbl">Highstreet Leasing</div>
...
async def handle_unit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not match:
        await update.message.reply_text("❌ Format not recognized. Try: B1-GF-05")
    ...
    await update.message.reply_text(
        f"✅ *{unit['unit']}* — {unit['type']}\n📐 Indoor: {unit['indoor']} sqm | Outdoor: {unit['outdoor'] or '—'} sqm\n💰 Base Rate: {fmt(unit['rate'])}/month\n🧾 Cheque: {fmt(unit['rate']+sc+mf)}\n\nPrepared for who?",
        parse_mode='Markdown')
    return ASK_TENANT
...
async def ask_agent(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"⏳ Sent to management for approval.", parse_mode='Markdown')
    return ConversationHandler.END
...
async def handle_approval(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'reject':
        ...
        await ctx.application.bot.send_message(pending['chat_id'], "❌ Offer not approved by management.")
        return
